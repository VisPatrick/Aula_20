...
# Distância = variância / (média ** 2)
# <= 10%: Baixa dispersão dos dados em relação à média
# > 10% e < 25%: Dispersão moderada dos dados em relação à média
# >= 25%: Alta dispersão dos dados em relação à média
# Se a distância for maior que 50%, os dados são considerados muito dispersos. Com isso, é possível identificar a variabilidade dos dados em relação à média e tomar decisões mais informadas.
